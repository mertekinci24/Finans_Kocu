...
export default function Assistant() {
  ...
  const pollParseAndAutoSummary = async (opts) => {
    ...
    const isAutoSummaryCandidate =
      ...
      originalText === 'Findeks raporumu analiz eder misin?' ||
      originalText === 'Yüklediğim dosyayı analiz eder misin?';

    if (!isAutoSummaryCandidate) return;
    if (autoSummarizedMessageIdsRef.current.has(messageId)) return;

    if (import.meta.env.DEV) {
      console.log('[POLL_PARSE] Starting poll', { messageId, storagePath, fileName, userId });
    }
